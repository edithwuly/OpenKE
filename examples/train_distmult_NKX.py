...
trainer.run()
distmult.save_checkpoint('./checkpoint/distmult_nkx.ckpt')
...
# predict
distmult.load_checkpoint('./checkpoint/distmult_nkx.ckpt')
predictor = Predictor(model = distmult, data_loader = test_dataloader, use_gpu = True)
links = predictor.run_link_prediction(type_constrain = False)
fout = open("nkx_distmult_prediction.txt", "w", encoding="utf-8")
