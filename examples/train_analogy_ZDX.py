...
trainer.run()
analogy.save_checkpoint('./checkpoint/analogy_zdx.ckpt')
...
# predict
analogy.load_checkpoint('./checkpoint/analogy_zdx.ckpt')
predictor = Predictor(model = analogy, data_loader = test_dataloader, use_gpu = True)
links = predictor.run_link_prediction(type_constrain = False)
fout = open("zdx_analogy_prediction.txt", "w", encoding="utf-8")
